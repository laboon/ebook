...
Both of these will return the same result for a given input.  The output of both can be observed via dynamic tests, and checked against expected values.  However, would anyone argue that the second method is superior?  It's overly complex; it's difficult to understand what it does; the variable names are meaningless.  Finding all that is wrong with the code is left as an exercise for the reader.

These two alternative methods illustrate the difference between __external quality__ and __internal quality__.  External quality is the kind of software quality which we quality analysts will usually concern themselves with most.  It measures the quality of the system as seen from the perspective of users of the software.  Examples of this would be meeting the requirements, producing the correct results, and responding to the user in an appropriate amount of time.

Internal quality is the quality of the system from a developers' perspective.  It measures how understandable and readable the code is, how maintainable it is, how well-documented it is, and other metrics which only those working on the system itself would know or care about.   Internal quality is often harder to determine and quantify, as code which seems very straightforward to one developer may seem very obtuse to a developer one cubicle over!

All things being equal, internal quality and external quality tend to be positively correlated.  It is difficult - although not impossible - for a program to be written poorly, undocumented, with little internal test coverage, and yet work reliably and always produce the correct results.  Similarly, it is possible for a program to very well-written, maintainable, and understandable, yet fails to work properly or meet the needs of the user.  When developing a system, higher internal quality is something to work for, but quality analysts will most often care about how the user sees the system and will thus tend to focus on external quality.

With dynamic tests, it may be difficult or impossible to determine any difference between the two `chirpify` methods above, and thus to get any sort of idea about the internal quality of the system.  However, using static testing methods such as code review, it's trivial to find issues in the second method.  In the chapter on Static Analysis, we will look more closely at techniques to determine software quality without executing code.
