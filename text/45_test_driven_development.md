...
After each red-green-refactor cycle, the developer should think of the next piece of functionality to add, and then loop back.  This cycling will continue until the software has done, a path of testing, coding, and refactoring that will eventually culminate in a complete product.  As a side effect, it will also include a solid test suite that is directly relevant to all the functionality of the program.
