...
Each turtle must be checked against every other turtle.
...
```
C(5, 3) = 5! (3! * (5 - 3)!) = 10 3-combinations
```
...
Let us return to our four-turtle Turtle Volleyball game.  Now, however, we don't care which turtle serves first - for the purposes of the game, Alan playing Bob is the same as Bob playing Alan.  In order to determine the number of tests
...
For each of these pairs, we want to have an entire truth table of the two elements covering all possibilities.  For example, to ensure that we test all of the possible combinations of bold and italic, we can make the following truth table:
