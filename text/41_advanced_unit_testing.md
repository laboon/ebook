...
public void testGoToCatCafe() {
    CatCafe cc = Mockito.mock(CatCafe.class);
    World w = new World();
...
Note, in this case, that there is no traditional assertion (e.g., `assertEquals`.  The test case ends with the execution steps...

Checking for multiple times...

Checking a method is never called

"An assertion on the executed code"
